...
class ModelSelectorScreen(ModalScreen[tuple[str, str] | None]):
    ...
    def action_add_custom_provider(self) -> None:
        """Open the custom provider add modal."""

        def _on_provider_saved(
            result: bool | tuple[bool, str, str | None],
        ) -> None:
    ...
    async def _reload_model_list(self) -> None:
        ...
        Reads fresh `_load_model_data()` off the main thread, then rebuilds
        the filtered model list and updates the display footer so the new
        provider appears immediately without dismissing the selector. The
        filter input value is preserved so the user doesn't lose their search
        term while the list rebuilds around it.
        """
        filter_value = self.query_one("#model-filter", Input).value
        data = await asyncio.to_thread(self._load_model_data, self._mode)
        self._unfiltered_models = data.all_models
        self._default_spec = data.default_spec
...
# ---------------------------------------------------------------------------
# DiscoverModelsScreen - multi-select model picker
# ---------------------------------------------------------------------------
...
class DiscoverModelsScreen(ModalScreen[list[str] | None]):
    ...
    Supports Select All / Select None via Ctrl+A / Ctrl+N, Enter to confirm
    selection, Escape to cancel without changes. Returns the list of selected
    model IDs as the modal result, or None when canceled.
    """

    BINDINGS: ClassVar[list[BindingType]] = [
        Binding("enter", "submit", "Save selection", show=False, priority=True),
        Binding("escape", "cancel", "Cancel", show=False, priority=True),
        Binding("ctrl+a", "select_all", "Select all", show=False, priority=True),
        Binding("ctrl+n", "select_none", "Select none", show=False, priority=True),
    ]
    ...
    DiscoverModelsScreen .discover-title {
        text-style: bold;
        color: $primary;
        text-align: center;
        margin-bottom: 1;
    }
    DiscoverModelsScreen .discover-description {
        color: $text-muted;
        margin-bottom: 1;
    }
    DiscoverModelsScreen .discover-hint {
        color: $text-muted;
        font-size: 85%;
        margin-bottom: 1;
    }
    DiscoverModelsScreen .discover-list {
        height: auto;
        min-height: 4;
        max-height: 20;
        scrollbar-gutter: stable;
    ...
    DiscoverModelsScreen .discover-button-bar {
        height: auto;
        dock: bottom;
        layout: horizontal;
        margin-top: 1;
    }
    DiscoverModelsScreen .discover-button {
        width: 1fr;
        height: 3;
        background: $background;
        color: $text;
    ...
    DiscoverModelsScreen .discover-button:hover {
        background: $boost;
    }
    DiscoverModelsScreen .discover-button.-primary {
        color: $primary;
    }
    """
    ...
    def compose(self) -> ComposeResult:
        ...
        from textual.widgets._option_list import Check

        yield Vertical(
            Static("Discover Models", classes="discover-title"),
            Static(
                f"{len(self._models)} model(s) found. Press Space to toggle selection.",
                classes="discover-description",
            ),
            Static(
                "Ctrl+A: Select All | Ctrl+N: Select None | Enter: Save",
                classes="discover-hint",
            ),
            OptionList(
                *[
                    Check(model_id, value=model_id in self._preselected)
                    for model_id in self._models
                ],
                classes="discover-list",
            ),
            Container(
                Button("Cancel", id="discover-cancel", classes="discover-button"),
                Button(
                    "Save Selection",
                    id="discover-save",
                    classes="discover-button -primary",
                ),
                classes="discover-button-bar",
            ),
        )
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "discover-save":
            self.action_submit()
        elif event.button.id == "discover-cancel":
            self.action_cancel()
    ...
    def action_submit(self) -> None:
        ...
        for option in option_list.options:
            if isinstance(option, Check) and option.value:
                selected.append(option.prompt)
    ...
    def action_select_all(self) -> None:
        ...
        for idx, option in enumerate(option_list.options):
            if isinstance(option, Check):
                option.value = True
                option_list.replace_option_prompt_at_index(idx, option)
...
# ---------------------------------------------------------------------------
# CustomProviderModalScreen - add/edit custom OpenAI-compatible provider
# ---------------------------------------------------------------------------
...
class CustomProviderModalScreen(ModalScreen[bool | tuple[bool, str, str | None]]):
    ...
    Returns `(success: bool, provider_id: str, default_model: str | None)`
    tuple when saved, or `False` when canceled. The tuple variant is always
    three values so callers can distinguish between save-only success and
    save + immediate switch (which dismisses the parent selector).
    """

    BINDINGS: ClassVar[list[BindingType]] = [
        Binding("enter", "submit", "Save provider", show=False, priority=True),
        Binding("escape", "cancel", "Cancel", show=False, priority=True),
    ]
    ...
    CustomProviderModalScreen .provider-title {
        text-style: bold;
        color: $primary;
        text-align: center;
        margin-bottom: 1;
    }
    CustomProviderModalScreen .provider-label {
        color: $text;
        margin-top: 1;
        margin-bottom: 0;
    }
    CustomProviderModalScreen .provider-input {
        margin-bottom: 1;
        border: solid $primary-lighten-2;
    }
    CustomProviderModalScreen .provider-input:focus {
        border: solid $primary;
    }
    CustomProviderModalScreen .provider-status {
        color: $text-muted;
        height: 1;
        margin-bottom: 1;
    }
    CustomProviderModalScreen .provider-error {
        color: $error;
        height: auto;
        margin-bottom: 1;
    }
    CustomProviderModalScreen .provider-button-bar {
        height: auto;
        dock: bottom;
        layout: horizontal;
        margin-top: 1;
    }
    CustomProviderModalScreen .provider-button {
        width: 1fr;
        height: 3;
        background: $background;
        color: $text;
    ...
    CustomProviderModalScreen .provider-button:hover {
        background: $boost;
    }
    CustomProviderModalScreen .provider-button.-primary {
        color: $primary;
    }
    CustomProviderModalScreen .provider-button.-discover {
        color: $accent;
    }
    """

    _MAX_MODEL_ID_LEN = 255
    """Maximum length for default_model input, matching existing provider-config fields."""

    def __init__(self, provider_id: str | None = None, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self._edit_provider_id = provider_id
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Vertical(
            Static("Add Custom Provider", classes="provider-title"),
            Static("Provider ID", classes="provider-label"),
            Input(
                placeholder="e.g., my_gateway (lowercase letters, digits hyphens underscore)",
                id="provider-id",
                classes="provider-input",
            ),
            Static("Display Name", classes="provider-label"),
            Input(
                placeholder="e.g., My Company Gateway",
                id="provider-display-name",
                classes="provider-input",
            ),
            Static("Base URL", classes="provider-label"),
            Input(
                placeholder="https://api.example.com/v1",
                id="provider-base-url",
                classes="provider-input",
            ),
            Static("API Key (optional)", classes="provider-label"),
            Input(
                placeholder="sk-... (leave blank if set via environment variable)",
                password=True,
                id="provider-api-key",
                classes="provider-input",
            ),
            Static("Default Model (optional)", classes="provider-label"),
            Input(
                placeholder="e.g., gpt-4o-mini",
                id="provider-default-model",
                classes="provider-input",
            ),
            Static("", id="provider-discovered-status", classes="provider-status"),
            Static("", id="provider-error", classes="provider-error"),
            Container(
                Button("Cancel", id="provider-cancel", classes="provider-button"),
                Button(
                    "Discover Models",
                    id="provider-discover",
                    classes="provider-button -discover",
                ),
                Button("Save", id="provider-save", classes="provider-button -primary"),
                classes="provider-button-bar",
            ),
        )
    ...
    def on_mount(self) -> None:
        ...
        Loads the existing TOML config and fills all fields except API key (which
        is never round-tripped through the UI).
        """
        import tomllib

        from deepagents_code.model_config import DEFAULT_CONFIG_PATH

        provider_id_input = self.query_one("#provider-id", Input)
        display_name_input = self.query_one("#provider-display-name", Input)
        base_url_input = self.query_one("#provider-base-url", Input)
        default_model_input = self.query_one("#provider-default-model", Input)

        if self._edit_provider_id is None:
            provider_id_input.focus()
            return
        ...
        config = providers.get(self._edit_provider_id, {})

        provider_id_input.value = self._edit_provider_id
        display_name_input.value = config.get("display_name", "")
        base_url_input.value = config.get("base_url", "")
        default_model_input.value = config.get("default_model", "")

        display_name_input.focus()

    def on_input_changed(self, event: Input.Changed) -> None:
        """Auto-fill display name from provider ID on first edit.

        Only triggers when the display name is empty and the provider ID is being
        edited for the first time. Transforms hyphens/underscores to spaces and
        applies title case. If the user has manually set a display name, subsequent
        ID changes do not overwrite it.
        """
        if event.input.id != "provider-id":
            return
        ...
        if provider_id:
            pretty = (
                provider_id.replace("-", " ").replace("_", " ").strip().title()
            )
            display_name_input.value = pretty

    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "provider-save":
            asyncio.create_task(self.action_submit())
        elif event.button.id == "provider-cancel":
            self.action_cancel()
        elif event.button.id == "provider-discover":
            asyncio.create_task(self.action_discover())

    async def action_discover(self) -> None:
        """Discover models from the provider's /models endpoint.

        Pushes a `DiscoverModelsScreen` with the fetched list. The callback
        stores the selected models and updates the default-model field with
        the first selected model (if not already set). The discovered-status
        line shows the count of selected models so the user knows the operation
        succeeded.
        """
        base_url_input = self.query_one("#provider-base-url", Input)
        api_key_input = self.query_one("#provider-api-key", Input)
        ...
        if self._edit_provider_id is not None and api_key is None:
            from deepagents_code import auth_store

            stored = auth_store.get_stored_key(self._edit_provider_id)
            if stored is not None:
                api_key = stored

        if not base_url:
            error_widget.update("Please enter a base URL first.")
            return
        ...
        try:
            models = await asyncio.to_thread(
                fetch_provider_models, base_url, api_key, timeout=10.0
            )
        ...
        def _on_models_selected(selected: list[str] | None) -> None:
            if selected is None:
                return
            self._discovered_models = selected
            self._update_discovered_status()
            default_model_input = self.query_one("#provider-default-model", Input)
            if selected and not default_model_input.value.strip():
                default_model_input.value = selected[0]

        self.app.push_screen(
            DiscoverModelsScreen(models, preselected=set(self._discovered_models)),
            _on_models_selected,
        )

    def _update_discovered_status(self) -> None:
        status_widget = self.query_one("#provider-discovered-status", Static)
        if self._discovered_models:
            status_widget.update(
                f"{len(self._discovered_models)} model(s) selected via Discover"
            )
        else:
            status_widget.update("")

    async def action_submit(self) -> None:
        """Validate and save the custom provider configuration.

        Performs basic validation (provider ID format, required fields, URL scheme,
        default-model length). Passes both discovered models and the default model
        to `save_custom_provider`. On success, dismisses with the tuple result;
        on failure, updates the error widget and stays open.
        """
        ...
        provider_id = provider_id_input.value.strip()
        display_name = display_name_input.value.strip()
        base_url = base_url_input.value.strip()
        api_key = api_key_input.value.strip() or None
        default_model = default_model_input.value.strip() or None

        if not provider_id:
            error_widget.update("Provider ID is required.")
            return
        if not re.match(r"^[a-z0-9][a-z0-9_-]*$", provider_id):
            error_widget.update(
                "Provider ID must start with a letter or digit and contain only "
        ...
        if parsed.scheme not in ("http", "https"):
            error_widget.update("Base URL must use http:// or https:// scheme.")
            return

        if default_model is not None and len(default_model) > self._MAX_MODEL_ID_LEN:
            error_widget.update(
                f"Default model must be less than {self._MAX_MODEL_ID_LEN} characters."
            )
            return

        error_widget.update("")

        # Merge discovered and default model; ensure default appears in models
        models_to_save = list(self._discovered_models) if self._discovered_models else None
        if default_model and models_to_save is not None and default_model not in models_to_save:
        ...
        if success:
            self.dismiss((True, provider_id, default_model))
        else:
            error_widget.update("Failed to save custom provider. Check logs.")
