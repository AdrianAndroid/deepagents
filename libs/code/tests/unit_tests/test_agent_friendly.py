...
class TestHelpScreenExamples:
    ...
    def test_list_help_has_examples(self) -> None:
        ...
        assert "zjcode list" in text
        assert "zjcode list --json" in text

    def test_skills_list_help_has_examples(self) -> None:
        text = self._render(show_skills_list_help)
        assert "Examples:" in text
        assert "zjcode skills list --project" in text
        assert "zjcode skills list --json" in text

    def test_skills_info_help_has_examples(self) -> None:
        text = self._render(show_skills_info_help)
        assert "Examples:" in text
        assert "zjcode skills info web-research" in text
        assert "zjcode skills info my-skill --project" in text

    def test_agents_help_has_examples(self) -> None:
        text = self._render(show_agents_help)
        assert "Examples:" in text
        assert "zjcode agents list" in text
        assert "zjcode agents reset --agent NAME" in text

    def test_update_help_has_examples(self) -> None:
        text = self._render(show_update_help)
        assert "Examples:" in text
        assert "zjcode update" in text
        assert "zjcode update --json" in text
...
class TestErrorMessageHints:
    ...
    def test_reset_source_not_found_has_hint(self, tmp_path: Path) -> None:
        ...
        output = buf.getvalue()
        assert "zjcode agents list" in output
...
class TestHelpScreenDriftExtended:
    ...
    def test_show_help_includes_mcp_subcommand(self) -> None:
        ...
        with patch("deepagents_code.ui.console", test_console):
            show_help()
        assert "zjcode mcp" in buf.getvalue()
    ...
    def test_show_help_includes_tools_subcommand(self) -> None:
        ...
        with patch("deepagents_code.ui.console", test_console):
            show_help()
        assert "zjcode tools" in buf.getvalue()
    ...
    def test_show_help_includes_install_subcommand(self) -> None:
        ...
        with patch("deepagents_code.ui.console", test_console):
            show_help()
        assert "zjcode install" in buf.getvalue()
