"""Unit tests for the `zjcode doctor` command."""
...
class TestDoctorArgs:
    ...
    def test_command_parsed(self) -> None:
        """`zjcode doctor` selects the doctor command."""
        with patch.object(sys, "argv", ["deepagents", "doctor"]):
            args = parse_args()
        assert args.command == "doctor"

    def test_json_flag(self) -> None:
        """`zjcode doctor --json` selects JSON output."""
...
class TestCollectSections:
    ...
    def test_diagnostics_reports_version(self) -> None:
        ...
        # Editable installs with stale metadata append a drift annotation;
        # only the prefix needs to match the running source version.
        assert labels["zjcode"].startswith(__version__)
...
class TestDiagnosticsVersionReport:
    ...
    def test_newer_exact_sdk_pin_is_informational_for_editable_sdk(self) -> None:
        """A newer exact zjcode pin is healthy for an editable main SDK checkout."""
        from packaging.requirements import Requirement
...
class TestRunDoctorCommand:
    ...
    def test_text_output_contains_sections(self) -> None:
        ...
        assert "zjcode config" in output
        assert "zjcode config get <key>" in output
        assert "zjcode --version" in output
        assert "zjcode -v" in output
...
class TestDoctorHelp:
    ...
    def test_help_renders(self) -> None:
        ...
        with patch("deepagents_code.ui.console", test_console):
            show_doctor_help()
        output = buf.getvalue()
        assert "zjcode doctor [options]" in output
